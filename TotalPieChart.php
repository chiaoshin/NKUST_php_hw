...
<body>
    <center>
        ...
        // $query = "SELECT SUM(amount) AS total, account_name FROM bank GROUP BY user_id";
        // $query = "SELECT SUM(amount) AS total, 'Bank' AS account_name FROM bank";
        $query = "(SELECT SUM(amount) As total, 'Bank' AS title FROM bank ) UNION ALL (SELECT SUM(cost) As total, 'Stock' As title FROM stock) UNION ALL (SELECT SUM(TWD_cost) As total, 'Cryptocurrency' As title FROM cryptocurrency)";
        // ,'Stock' AS stock_name FROM stock
                    ...
                    data: [
                        ...
                        if ($getData->num_rows > 0) {
                            while ($row = $getData->fetch_object()) {
                                $data .= '{ name:"' . $row->title . '",y:' . $row->total . '},';
                            }
                        }
                        echo $data;
                        ?>
        ...
        <div class="footer">
            <div class="fs-5 text-center mt-1 text-muted">
