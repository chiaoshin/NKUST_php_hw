...
if (isset($_GET['delete_id'])) {
    ...
    $sql_query = "DELETE FROM cryptocurrency WHERE cryptocurrency_id=" . $_GET['delete_id'];
...
<head>
    ...
    <script type="text/javascript">
        function edt_id(id) {
            if (confirm('確定要修改嗎?\u000aSure to Edit?')) {
                window.location.href = 'cryptocurrency_edit_data.php?edit_id=' + id;
            }
        }

        function delete_id(id) {
            if (confirm('確定要刪除嗎?\u000aSure to Delete?')) {
                window.location.href = 'cryptocurrency_main.php?delete_id=' + id;
            }
        }
...
<body>
    <center>
        ...
        $query = "SELECT * FROM cryptocurrency"; // get the records on which pie chart is to be drawn
                    ...
                    data: [
                        ...
                        if ($getData->num_rows > 0) {
                            while ($row = $getData->fetch_object()) {
                                $data .= '{ name:"' . $row->cryptocurrency_name . '",y:' . $row->TWD_cost . '},';
                            }
                        }
                        echo $data;
                        ?>
            ...
            <div id="content">
                <table align="center" style="border: 10px white solid;">
                    <tr>
                        ...
                        <th colspan="5" class="thcolor"><a href="cryptocurrency_add_data.php" style="background-color:#EAC100;"><span class=" material-symbols-sharp">add</span>新增資料</a></th>
                    </tr>
                    <th style="color:#0072E3;background-color:#EAC100;border: 5px white solid;">【貨幣名稱 cryptocurrency Name】</th>
                    <th style="color:#0072E3;background-color:#EAC100;border: 5px white solid;">【買入價格 Price】</th>
                    <th style="color:#0072E3;background-color:#EAC100;border: 5px white solid;">【買入數量 Amount】</th>
                    ...
                    <th style="color:#0072E3;background-color:#EAC100;border: 5px white solid;">【台幣成本 TWD Cost】</th>
                    ...
                    $sql_query = "SELECT cryptocurrency_id,cryptocurrency_name,FORMAT(price,2) AS price,FORMAT(amount,5) AS amount,FORMAT(handlingFee,2) AS handlingFee,FORMAT(cost,2) AS cost,FORMAT(TWD_cost,2) AS total, note FROM cryptocurrency";
